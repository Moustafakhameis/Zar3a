import { Notification, User, Product, Inquiry } from '../models/index.js';
...
export const createInquiry = async (req, res) => {
  try {
    ...
    if (!['FARMER', 'BUYER', 'ADMIN'].includes(user.role)) {
      return res.status(403).json({ message: 'Only Farmers, Buyers, and Admins can request sensor quotes.' });
    }
    ...
    return res.status(201).json({ message: 'Quote request submitted successfully. An Admin will contact you shortly.', inquiry });
  } catch (err) {
