...
export default function ProductsDashboard() {
  ...
  const loadProducts = async () => {
    ...
    try {
      ...
      // Filter products: ADMIN sees all, others see ONLY those belonging to the logged-in user
      const userProducts = user.role === "ADMIN" 
        ? fetchedProducts 
        : fetchedProducts.filter(p => p.userId === user.id || p.User?.id === user.id);
          ...
          <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
            {filteredProducts.length > 0 && (
              <button
                ...
                className="flex-1 md:flex-none px-6 py-3.5 rounded-2xl font-black text-[11px] text-white bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 shadow-lg shadow-blue-500/20 flex items-center justify-center gap-2 transition-all active:scale-95 uppercase tracking-widest"
              >
            ...
            <button
              ...
              className="flex-1 md:flex-none px-6 py-3 bg-surface-secondary dark:bg-slate-800 text-slate-700 dark:text-slate-200 rounded-2xl font-bold text-xs hover:bg-gray-200 dark:hover:bg-slate-700 transition"
            >
              {t("prodDash.refresh")}
            </button>
            <button
