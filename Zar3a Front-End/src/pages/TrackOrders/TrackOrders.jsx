                    ...
                    {item.status === 'CANCELLED' ? (
                      ...
                      <div className="relative flex justify-between items-center w-full px-2 sm:px-8">
                        ...
                        {['PENDING', 'PROCESSING', 'SHIPPED', 'DELIVERED'].map((step, idx) => {
                          ...
                          return (
                            <div key={step} className="relative z-10 flex flex-col items-center gap-1 sm:gap-3 bg-surface-card dark:bg-slate-900 px-0.5 sm:px-2">
                              <div className={`w-6 h-6 sm:w-8 sm:h-8 rounded-full flex items-center justify-center border-2 sm:border-4 border-white dark:border-slate-900 transition-all duration-500 shadow-sm ${isCompleted ? 'bg-emerald-500 text-white' : 'bg-slate-200 dark:bg-slate-700 text-text-disabled'} ${isActive ? 'ring-4 ring-emerald-500/30 scale-110' : ''}`}>
                                {isCompleted ? <LuCheck size={12} className="stroke-[3] sm:w-3.5 sm:h-3.5" /> : <span className="text-[9px] sm:text-[10px] font-bold">{idx + 1}</span>}
                              </div>
                              <span className={`text-[8px] sm:text-[10px] font-black uppercase tracking-tighter sm:tracking-widest text-center w-[50px] sm:w-auto break-words leading-tight ${isActive ? 'text-primary-base dark:text-emerald-400' : isCompleted ? 'text-slate-700 dark:text-slate-300' : 'text-text-disabled'}`}>
