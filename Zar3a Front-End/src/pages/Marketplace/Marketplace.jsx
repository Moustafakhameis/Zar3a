...
import { useAllProductsQuery, useCreateProductMutation } from '../../hooks/queries/useMarketplace';
...
const Marketplace = () => {
  ...
  const { data: queryProducts, isLoading: loadingProducts, isError } = useAllProductsQuery();
  const createProductMutation = useCreateProductMutation();
  const products = queryProducts && queryProducts.length > 0 ? queryProducts.map(normalizeProduct) : DEFAULT_PRODUCTS;
  ...
  return (
    ...
    <div className="max-w-7xl mx-auto space-y-6 pb-20 px-4 text-left">
      ...
      <div className="flex flex-col xl:flex-row justify-between items-center gap-4 sm:gap-6 bg-surface-card dark:bg-slate-900 p-3 sm:p-4 rounded-[2.5rem] shadow-sm border border-border-default dark:border-slate-800">
        <div className="flex w-full xl:w-auto overflow-x-auto no-scrollbar bg-surface-secondary dark:bg-slate-800 p-1 sm:p-1.5 rounded-full relative">
          <button
            onClick={() => {
              setActiveTab("shop");
            ...
            className={`relative flex-1 xl:flex-none min-w-max px-4 sm:px-6 py-2.5 sm:py-3 rounded-full font-black text-xs sm:text-sm whitespace-nowrap transition-all z-10 ${
              activeTab === "shop"
                ? "text-white"
        ...
        <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 w-full xl:w-auto">
          {user && (
            <button
              ...
              className="relative flex items-center justify-center gap-2 sm:gap-3 bg-gray-900 dark:bg-slate-800 text-white px-6 sm:px-8 py-3.5 sm:py-4 rounded-full font-black hover:scale-105 transition-transform shadow-xl w-full sm:w-auto whitespace-nowrap text-sm sm:text-base"
            >
          ...
          {canCreate && (
            <button
              ...
              className="flex items-center justify-center gap-2 bg-primary-base text-white px-6 sm:px-8 py-3.5 sm:py-4 rounded-full font-black hover:bg-primary-hover transition-transform shadow-xl w-full sm:w-auto whitespace-nowrap text-sm sm:text-base"
            >
              <LuPlus size={20} className="w-4 h-4 sm:w-5 sm:h-5" /> {t("market.addProduct")}
            </button>
          )}
        </div>
      </div>
